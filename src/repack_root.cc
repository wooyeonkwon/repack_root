...
bool IsLeft(int ch) { return (ch >= 1 && ch <= 32); }
bool IsRight(int ch) { return (ch >= 33 && ch <= 64); }
int OppChannel(int ch) { return 65 - ch; }
...
long long CountEvtnumDrops(TTree* tin, TDC1Rec& rec) {
  const Long64_t n = tin->GetEntries();
  int prev = -1;
  long long drops = 0;
  for (Long64_t i = 0; i < n; ++i) {
    tin->GetEntry(i);
    if (prev != -1 && rec.evtnum < prev) {
      drops++;
    }
    prev = rec.evtnum;
  }
...
void FinalizeEvent(int evtnum, EventBuffers& ev) {
...
int RunRepackRoot(int argc, char** argv) {
  if (argc < 3) {
